...
Все пользовательские настройки PulseColor описаны в `handleEvents.json` и изменяются через приложение PulseSync. Runtime получает начальные значения через `window.pulsesyncApi`, подписывается на их изменения и применяет их без собственной панели настроек внутри Яндекс Музыки.

## Разработка и сборка

PulseColor использует структуру официального `PulseSync-ExampleAddon`: точка входа находится в `src/main.ts`, статические файлы аддона — в `addon/`, а `addon.config.mjs` служит источником для генерируемого `metadata.json`.
...
- `yarn dev` собирает без минификации прямо в установленную папку PulseSync и следит за изменениями;
- `yarn build` создаёт готовый пакет в `dist/419c58cb-75c5-48bf-b053-933f3b81190f`;
- `yarn sync` переносит готовый пакет в PulseSync, сохраняя пользовательский `pulsesync.settings.json`;
- `yarn build:sync` последовательно выполняет сборку и синхронизацию.

Итоговый пакет содержит единые `script.js` и `script.css`, поэтому runtime не использует динамическую загрузку отдельных JS-файлов.
